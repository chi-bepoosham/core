<?php

return [
    'defaults' => [
        'success' => 'با موفقیت انجام شد',
        'failed' => 'خطایی رخ داده است',
        'store_success' => 'داده ی مورد نظر با موفقیت ذخیره شد',
        'store_failed' => 'خطایی رخ داد و ذخیره انجام نشد',
        'show_success' => 'داده مورد نظر با موفقیت یافت شد',
        'show_failed' => 'خطایی رخ داد و داده یافت نشد',
        'index_success' => 'لیست اطلاعات به شرح زیر است',
        'index_failed' => 'خطایی رخ داد',
        'delete_success' => 'حذف با موفقیت انجام شد',
        'delete_failed' => 'خطایی رخ داد و حذف انجام نشد',
        'destroy_success' => 'با موفقیت حذف انجام شد',
        'destroy_failed' => 'خطایی رخ داد و حذف انجام نشد',
        'update_success_list' => 'ویرایش اطلاعات با موفقیت انجام شد',
        'update_success' => 'ویرایش با موفقیت انجام شد',
        'update_failed' => 'ویرایش به خطا خورد و انجام نشد',
        'update_failed_list' => 'ویرایش اطلاعات به خطا خورد و انجام نشد',
        'upload_success' => 'آپلود تصویر با موفقیت انجام شد',
        'upload_failed' => 'آپلود تصویر انجام نشد',
        'user_creator_id' => 'شناسه کاربر ایجاد کننده',
        'not_found' => 'اطلاعات خواسته شده یافت نشد',
        'app_updating' => 'سامانه در حال به روز رسانی می باشد',
        'user_unauthenticated' => 'کاربر احراز هویت نشده است',
        'exceptions' => [
            '500' => 'خطا در سرور',
            '403' => 'شما اجازه دسترسی به این بخش را ندارید',
        ],
    ],
    'validation' => [
        'username' => 'قالب :attribute شما نادرست است',
        'national_code' => 'کد ملی وارد شده معتبر نیست',
        'user_name_or_password_incorrect' => 'نام کاربری یا رمز عبور شما اشتباه است'
    ],
    'user' => [
        'not_exist' => 'کاربری با این مشخصات در سیستم وجود ندارد.',
        'email_exist' => 'ایمیل انتخاب شده در سیستم وجود دارد.',
        'mobile_exist' => 'شماره موبایل انتخاب شده در سیستم وجود دارد.',
        'mobile_not_exist' => 'شماره موبایل انتخاب شده در سیستم وجود ندارد. لطفا ابتدا ثبت نام کنید',
        'send_otp_successfully' => 'کد تایید با موفقیت ارسال شد',
        'logout_successfully' => 'شما با موفقیت از سیستم خارج شدید',
        'send_otp_failed' => 'خطایی هنگام ارسال کد تایید پیش آمده است. لطفا دوباره تلاش کنید',
        'otp_check_allow_send_code' => 'کد تایید برای شما ارسال شده است . لطفا چند دقیقه دیگر امتحان کنید',
        'register_exception' => 'هنگام ثبت نام خطایی پیش آمده است. لطفا دوباره تلاش کنید',
        'create_user_exception' => 'هنگام ثبت کاربر جدید خطایی پیش آمده است. لطفا دوباره تلاش کنید',
        'user_not_exist' => 'کاربری با این مشخصات در سیستم وجود ندارد',
        'body_type_not_detected' => 'اطلاعات فرم بدن شما در حال پردازش است. لطفا کمی دیگر تلاش کنید',
        'oauth_google_exception' => 'خطایی در برقراری ارتباط با Google پیش آمده است. لطفا دوباره تلاش کنید',
    ],
    'shop' => [
        'not_exist' => 'فروشگاهی با این مشخصات در سیستم وجود ندارد.',
        'product_not_exist' => 'محصولی با این مشخصات در سیستم وجود ندارد.',
        'email_exist' => 'ایمیل انتخاب شده در سیستم وجود دارد.',
        'mobile_exist' => 'شماره موبایل انتخاب شده در سیستم وجود دارد.',
        'mobile_not_exist' => 'شماره موبایل انتخاب شده در سیستم وجود ندارد. لطفا ابتدا ثبت نام کنید',
        'register_exception' => 'هنگام ثبت نام خطایی پیش آمده است. لطفا دوباره تلاش کنید',
        'create_shop_exception' => 'هنگام ثبت فروشگاه جدید خطایی پیش آمده است. لطفا دوباره تلاش کنید',
        'not_access_register_order_shop_not_found' => 'فروشگاه :shop در حال حاضر در دسترس نمی‌باشد',
        'not_access_register_order_product_not_found' => 'محصول :product در حال حاضر در دسترس نمی‌باشد',
        'not_access_register_order_product_size_not_found' => 'سایز انتخابی محصول :product در حال حاضر در دسترس نمی‌باشد',
        'not_access_register_order_user_address' => 'شما اجازه ثبت سفارش برای این آدرس را ندارید',
        'not_access_pay_order_progress_status' => 'امکان پرداخت سفارش به دلیل وضعیت آن، امکان پذیر نیست',
        'not_access_update_order_delivered' => 'سفارش شما تحویل شده است و امکان ویرایش وضعیت را ندارید. در صورت بروز مشکل با پشتیبانی تماس بگیرید.',
        'not_access_update_order_returned' => 'سفارش شما برگشت داده شده است و امکان ویرایش وضعیت را ندارید. در صورت بروز مشکل با پشتیبانی تماس بگیرید.',
        'not_access_update_order_canceled' => 'سفارش شما لغو شده است و امکان ویرایش وضعیت را ندارید. در صورت بروز مشکل با پشتیبانی تماس بگیرید.',
        'product_marked' => 'محصول با موفقیت به نشان شده ها اضافه شد',
        'product_unmarked' => 'محصول با موفقیت از نشان شده ها حذف شد',
        'payment_order_successfully' => 'پرداخت با موفقیت انجام شد',
    ],
    'wallet' => [
        'not_exist' => 'کیف پول با این مشخصات در سیستم وجود ندارد.',
    ]
];
